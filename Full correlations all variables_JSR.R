...
Predicted_CPUE<--11075.5+(cor.mat[1:25,4]*3555.8)
Predicted_CPUE
...
Pred.CPUE.Nickmodel1<-3449.681+(cor.mat[1:25,12]*-5.378)
lines(Newdata$year[3:27],Pred.CPUE.Nickmodel1, col="blue")

#Predicting CPUE from 1987-2013 using model 1.1
Pred.CPUE.Wilbermodel<--11075.5+(cor.mat[28:54,4]*3555.8)
...
Pred.CPUE.Nickmodel<-3449.7+(cor.mat[28:54,12]*-5.39)
...
Pred_CPUE_X1_recent<--5718.9+(1844.4*cor.mat[28:54,4])

Pred_CPUE_recent<-1465.1+(-1.0*cor.mat[28:54,12])
...
summary(lm.Nick)
Pred.lm.Nick<-2895.701+(-6.888*cor.mat[1:54,12])
...
summary(lm.Wilber)
Pred.lm.Wilber<--22236.1+(cor.mat[1:54,4]*6256)
...
CPUE.trips<-Newdata$adj_landings[30:56]/Trips$Trips[2:28]
S<-rep(NA,29)

CPUE.t<-c(S,CPUE.trips)
CPUE.o<-Newdata$CPUE
